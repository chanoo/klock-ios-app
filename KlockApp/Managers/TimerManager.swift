...
class TimerManager {
    ...
    func addTimer(type: String, completion: @escaping (TimerModel?) -> Void) {
        ...
        switch timerType {
        case .focus:
            let req = ReqFocusTimer(seq: seq, name: "시험")
            focusTimerRemoteService.create(data: req)
                ...
                .store(in: &cancellables)
        case .pomodoro:
            let req = ReqPomodoroTimer(seq: seq, name: "시험", focusTime: 25, breakTime: 5, cycleCount: 4)
            pomodoroTimerRemoteService.create(data: req)
                ...
                .store(in: &cancellables)
        case .exam:
            let req = ReqExamTimer(seq: seq, name: "시험", startTime: "2023-01-01T08:40:00.000000", duration: 80, questionCount: 45)
            examTimerRemoteService.create(data: req)
                ...
                .store(in: &cancellables)
        case .auto:
            let req = ReqAutoTimer(seq: seq, name: "시험")
